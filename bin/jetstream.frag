...
float nse3d(in vec3 x)
{
    ...
    vec2 rg = texture2D(iChannel15, (uv + 0.5) / 256.0, -100.0).yx;
    return mix(rg.x, rg.y, f.z);
}

float nse(vec2 p)
{
    return texture2D(iChannel15, p).x;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float sd = sin(fragCoord.x * 0.01 + fragCoord.y * 3.333333333 + iGlobalTime) * 1298729.146861;
