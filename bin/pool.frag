...
{
    return 24.0*pow( textureCube( skybox2, rd ).xyz, vec3(2.2) );
}

vec3 doEnvironmentBlurred( in vec3 rd )
{
    return 9.0*pow( textureCube( skybox3, rd ).xyz, vec3(2.2) );
}
...
vec4 doMaterial( in vec3 pos, in vec3 nor, float ma )
{
    ...
    if( ma<0.5 ) { mate = vec4(0.01,0.20,0.03,0.0)*1.25; 
                   mate.xyz *= 0.78 + 0.22*texture2D( iChannel2, 0.1*pos.xz ).x; 
                 }
    return mate;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for( int a=0; a<40; a++ )
    {
        vec4 rrr = texture2D( iChannel15, (fragCoord.xy +0.5+3.3137*float(a))/iChannelResolution[0].xy  ).xzyw;
