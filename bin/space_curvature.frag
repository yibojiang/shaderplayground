...
{
    vec3 col = vec3(0.0);
         col += 0.5*pow( fancyCube( iChannel8, d, 0.05, 5.0 ).zyx, vec3(2.0) );
         col += 0.2*pow( fancyCube( iChannel8, d, 0.10, 3.0 ).zyx, vec3(1.5) );
         col += 0.8*vec3(0.80,0.5,0.6)*pow( fancyCube( iChannel8, d, 0.1, 0.0 ).xxx, vec3(6.0) );
    float stars = smoothstep( 0.3, 0.7, fancyCube( iChannel8, d, 0.91, 0.0 ).x );
...
vec3 render( in vec3 ro, in vec3 rd )
{
    ...
    if( t>0.0 )
    {
        ...
        float l = fancyCube( iChannel2, tnor, 0.03, 0.0 ).x;
        l += -0.1 + 0.3*fancyCube( iChannel2, tnor, 8.0, 0.0 ).x;

        vec3 sea  = mix( vec3(0.0,0.07,0.2), vec3(0.0,0.01,0.3), fre );
        sea *= 0.15;

        vec3 land = vec3(0.02,0.04,0.0);
        land = mix( land, vec3(0.05,0.1,0.0), smoothstep(0.4,1.0,fancyCube( iChannel2, tnor, 0.1, 0.0 ).x ));
        land *= fancyCube( iChannel2, tnor, 0.3, 0.0 ).xyz;
        land *= 0.5;

        float los = smoothstep(0.45,0.46, l);
        mat = mix( sea, land, los );

        vec3 wrap = -1.0 + 2.0*fancyCube( iChannel8, tnor2.xzy, 0.025, 0.0 ).xyz;
        float cc1 = fancyCube( iChannel8, tnor2 + 0.2*wrap, 0.05, 0.0 ).y;
