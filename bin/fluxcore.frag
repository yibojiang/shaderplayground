...
vec3 RayTrace(in vec2 fragCoord )
{
  ...
  camPos = vec3(cos(my)*cos(mx),sin(my),cos(my)*sin(mx))*8.35;
    ...
    } else if (localTime < t6)
    {
        ...
        float smooth1 = smoothstep(0.0, 1.0, saturate(alpha*1.8-0.1));
        animStructure = 1.0-smooth1;
        fade = saturate(time);
        fade *= saturate(t6 - localTime);
        camPos = vec3(10.0, -0.95+smooth1*1.0, 0.0);
        camPos.x -= alpha * 6.8;
        camUp=normalize(vec3(0,1.0-smooth1,0.0+smooth1));
        camLookat=vec3(0,-0.0,-0.0);
    }
#endif
