...
float noise( in vec2 x )
{
    ...
    vec2 f = fract(x);
    f = f*f*(3.0-2.0*f);
    float a = texture2D(iChannel11,(p+vec2(0.5,0.5))/256.0).x;
    float b = texture2D(iChannel11,(p+vec2(1.5,0.5))/256.0).x;
    float c = texture2D(iChannel11,(p+vec2(0.5,1.5))/256.0).x;
    float d = texture2D(iChannel11,(p+vec2(1.5,1.5))/256.0).x;
