...
// Computes analytical normals (left side of the screen) for a value-noise based fbm(), 
// and compares them to the numerically computer normals (right side of the screen) which
...
// while numerical normals are easier to filter for antialiasing.
//
...
vec4 noised( in vec3 x )
{
  ...
  float f = hash(n+114.0);
    ...
    return vec4( k0 + k1*u.x + k2*u.y + k3*u.z + k4*u.x*u.y + k5*u.y*u.z + k6*u.z*u.x + k7*u.x*u.y*u.z, 
                 6.0*w*(1.0-w) * vec3( k1 + k4*u.y + k6*u.z + k7*u.y*u.z,
...
vec4 fbmd( in vec3 x )
{
  ...
  float f = 1.0;
    ...
    for( int i=0; i<8; i++ )
    {
        ...
        d += b*n.yzw*f*scale; // accumulate derivatives (note that in this case b*f=1.0)
