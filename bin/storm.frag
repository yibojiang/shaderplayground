...
float noise( in vec2 x )
{
	...
	vec2 uv = (p.xy) + f.xy;
	return texture( iChannel0, (uv+ 0.5)/256.0, -100.0 ).x;
}
...
vec3 raymarch( in vec3 ro, in vec3 rd, in vec2 ani, in vec2 pixel )
{
	...
	float t = 0.03*texture( iChannel0, pixel.xy/256.0 ).x;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// camera parameters
	// vec4 camPars = texture( iChannel0, floor(1.0+iGlobalTime/5.5)*vec2(5.0,7.0)/iChannelResolution[0].xy );
	vec4 camPars = texture( iChannel0, floor(1.0+iGlobalTime/5.5)*vec2(5.0,7.0)/256.0 );
