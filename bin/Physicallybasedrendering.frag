...
vec3 getColor(vec3 ray) {
    ...
    if(dist > 0.0) {
        ...
        vec3 light_color = textureCube(skybox0,vec3(1.0,0.0,0.0)).xyz * 1.2;
                
                
        // IBL
        vec3 ibl_diffuse = textureBlured(skybox0,normal);
        vec3 ibl_reflection = textureBlured(skybox0,reflect(ray,normal));
        ...
        // reflection        
        vec3 refl = textureCube(skybox0,reflect(ray,normal)).xyz;
        ...
        // diffuse
        vec3 diff = ibl_diffuse * vec3(1.0,0.25,0.0);
        ...
        return textureCube(skybox0,ray).xyz;
    }
}
