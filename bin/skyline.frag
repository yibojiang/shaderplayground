...
vec3 RayTrace(in vec2 fragCoord )
{
    ...
    if ((t <= maxDepth) || (t == alpha))
	{
        ...
        if (distAndMat.y >= 100.0)
        {
            ...
            // finalColor += saturate(textureCube(iChannel0, ref).xyz-0.35)*0.15*max(0.2,sunShadow);
        }
        ...
        if (windowRef != 0.0)
        {
            ...
            // finalColor += saturate(textureCube(iChannel0, ref).xyz-0.35)*0.15*max(0.25,sunShadow)*windowRef;
        }
        finalColor *= 0.9;
